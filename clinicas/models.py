...
class Sucursal(ClaseModelo):
    ...
    def clean(self):
        ...
        if self.nombre:
            self.nombre = self.nombre.strip().title()
# models.py para app clinicas
from django.db import models
from bases.models import ClaseModelo

class Clinica(ClaseModelo):
    nombre = models.CharField(max_length=150, unique=True, verbose_name='Nombre de la Clínica')
    # Sobrescribir campos de ClaseModelo para evitar conflicto related_name
    uc = models.ForeignKey('auth.User', on_delete=models.SET_NULL, null=True, blank=True, related_name='clinicas_uc', verbose_name='Usuario Creador')
    um = models.IntegerField(null=True, blank=True, verbose_name='Usuario Modificador')
    direccion = models.TextField(verbose_name='Dirección', help_text='Dirección completa de la clínica principal')
    telefono = models.CharField(max_length=20, verbose_name='Teléfono', help_text='Teléfono de contacto principal')
    email = models.EmailField(verbose_name='Email', help_text='Email de contacto de la clínica')
    ruc = models.CharField(max_length=20, blank=True, null=True, verbose_name='RUC/NIT/CUIT', help_text='Número de identificación fiscal según país')
    razon_social = models.CharField(max_length=200, blank=True, null=True, verbose_name='Razón Social', help_text='Nombre legal de la empresa para facturas')
    representante_legal = models.CharField(max_length=150, blank=True, null=True, verbose_name='Representante Legal', help_text='Nombre completo del representante legal')
    PAISES_CHOICES = [
        ('EC', 'Ecuador'), ('PE', 'Perú'), ('CO', 'Colombia'), ('MX', 'México'), ('CL', 'Chile'), ('AR', 'Argentina'),
    ]
    MONEDA_CHOICES = [
        ('USD', 'Dólar Estadounidense'), ('PEN', 'Sol Peruano'), ('COP', 'Peso Colombiano'), ('MXN', 'Peso Mexicano'), ('CLP', 'Peso Chileno'), ('ARS', 'Peso Argentino'),
    ]
    pais = models.CharField(max_length=2, choices=PAISES_CHOICES, default='EC', verbose_name='País', help_text='País donde opera la clínica')
    moneda = models.CharField(max_length=3, choices=MONEDA_CHOICES, default='USD', verbose_name='Moneda', help_text='Moneda de operación')
    zona_horaria = models.CharField(max_length=50, default='America/Guayaquil', verbose_name='Zona Horaria', help_text='Zona horaria del país (ej: America/Guayaquil)')
    logo = models.ImageField(upload_to='clinicas/logos/', blank=True, null=True, verbose_name='Logo', help_text='Logo de la clínica (opcional)')
    sitio_web = models.URLField(blank=True, null=True, verbose_name='Sitio Web', help_text='URL del sitio web oficial (opcional)')
    class Meta:
        verbose_name = 'Clínica'
        verbose_name_plural = 'Clínicas'
        ordering = ['nombre']
...
class Sucursal(ClaseModelo):
    clinica = models.ForeignKey(Clinica, on_delete=models.CASCADE, related_name='sucursales', verbose_name='Clínica')
    # Sobrescribir campos de ClaseModelo para evitar conflicto related_name
    uc = models.ForeignKey('auth.User', on_delete=models.SET_NULL, null=True, blank=True, related_name='sucursales_uc', verbose_name='Usuario Creador')
    um = models.IntegerField(null=True, blank=True, verbose_name='Usuario Modificador')
    nombre = models.CharField(max_length=150, verbose_name='Nombre de la Sucursal')
    direccion = models.TextField(verbose_name='Dirección', help_text='Dirección completa de la sucursal')
    telefono = models.CharField(max_length=20, verbose_name='Teléfono', help_text='Teléfono de contacto de la sucursal')
    horario_apertura = models.TimeField(verbose_name='Hora de Apertura', help_text='Hora de apertura (formato 24hrs)')
    horario_cierre = models.TimeField(verbose_name='Hora de Cierre', help_text='Hora de cierre (formato 24hrs)')
    dias_atencion = models.CharField(max_length=50, verbose_name='Días de Atención', help_text='Ej: Lunes a Viernes, Lunes a Sábado', default='Lunes a Viernes')
    sabado_horario_apertura = models.TimeField(blank=True, null=True, verbose_name='Sábado - Hora de Apertura', help_text='Dejar vacío para usar el horario general')
    sabado_horario_cierre = models.TimeField(blank=True, null=True, verbose_name='Sábado - Hora de Cierre', help_text='Dejar vacío para usar el horario general')
    domingo_horario_apertura = models.TimeField(blank=True, null=True, verbose_name='Domingo - Hora de Apertura', help_text='Dejar vacío para usar el horario general')
    domingo_horario_cierre = models.TimeField(blank=True, null=True, verbose_name='Domingo - Hora de Cierre', help_text='Dejar vacío para usar el horario general')
    class Meta:
        verbose_name = 'Sucursal'
        verbose_name_plural = 'Sucursales'
        ordering = ['clinica', 'nombre']
